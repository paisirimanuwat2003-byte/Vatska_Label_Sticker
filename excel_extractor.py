...
root = tk.Tk()
root.title("Excel Data Extractor For Labeling")
root.minsize(400, 600)  # Set a minimum size for the window

# 1. Load the PNG image file
img = tk.PhotoImage(file="vatska_icon.png")

root.iconphoto(True, img)  # Ensure you have an icon.png in the same directory
...
# --- THE SCROLLBAR FIX ---
# Create a frame to hold the text box and the scrollbar together
frame = tk.Frame(root)
...
# Create the text box and link its yscrollcommand to the scrollbar
text_preview = tk.Text(frame, wrap=tk.WORD, width=45, height=10, font=("Consolas", 10), yscrollcommand=scrollbar1.set)
text_preview.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)

# Link the scrollbar back to the text box
scrollbar1.config(command=text_preview.yview)
# -------------------------

text_preview.insert(tk.END, "กรุณาเลือกไฟล์ Excel ที่ต้องการคัดลอกข้อมูล\n\nหลังจากเลือกไฟล์แล้ว ปุ่มคัดลอกจะถูกเปิดใช้งาน\n\n")

# The 8 Copy Buttons (Disabled by default until a file is loaded)
btn_copy1 = tk.Button(root, text="ถุงเล็ก (x.1 หรือ x)", command=copy_1, bg="lightgreen", state="disabled")
btn_copy1.pack(pady=5, fill='x', padx=50)

btn_copy2 = tk.Button(root, text="ถุงเล็ก (x.2)", command=copy_2, bg="lightgreen", state="disabled")
btn_copy2.pack(pady=5, fill='x', padx=50)

btn_copy3 = tk.Button(root, text="ถุงเล็ก (x.3)", command=copy_3, bg="lightgreen", state="disabled")
btn_copy3.pack(pady=5, fill='x', padx=50)

btn_copy4 = tk.Button(root, text="ถุงเล็ก (x.4)", command=copy_4, bg="lightgreen", state="disabled")
btn_copy4.pack(pady=5, fill='x', padx=50)

btn_copy5 = tk.Button(root, text="ถุงเล็ก (x.5)", command=copy_5, bg="lightgreen", state="disabled")
btn_copy5.pack(pady=5, fill='x', padx=50)

btn_copy6 = tk.Button(root, text="ถุงใหญ่", command=copy_6, bg="lightgreen", state="disabled")
btn_copy6.pack(pady=5, fill='x', padx=50)

btn_copy7 = tk.Button(root, text="กล่อง", command=copy_7, bg="lightgreen", state="disabled")
btn_copy7.pack(pady=5, fill='x', padx=50)

btn_copy8 = tk.Button(root, text="Barcode", command=copy_8, bg="lightblue", state="disabled")
btn_copy8.pack(pady=5, fill='x', padx=50)
